...
#define LCD_BL_ON() HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12, GPIO_PIN_RESET)
#define LCD_BL_OFF() HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12, GPIO_PIN_SET)

#define LCD_BASE0        		((uint32_t)0x60000000)
#define LCD_BASE1        		((uint32_t)0x60020000)

#define LCD_CmdWrite(command)	*(__IO uint16_t *) (LCD_BASE0) = (command)
#define LCD_DataWrite(data)		*(__IO uint16_t *) (LCD_BASE1) = (data)
#define	LCD_StatusRead()		*(__IO uint16_t *) (LCD_BASE0) //if use read  Mcu interface DB0~DB15 needs increase pull high
#define	LCD_DataRead()			*(__IO uint16_t *) (LCD_BASE1) //if use read  Mcu interface DB0~DB15 needs increase pull high
